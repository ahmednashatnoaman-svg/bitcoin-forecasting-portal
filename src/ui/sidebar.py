...
# OHLC options presented as styled radio buttons
OHLC_OPTIONS = ['Close', 'Open', 'High', 'Low']
    ...
    with st.sidebar:
        ...
        with st.expander("🔬 Expert Configurations", expanded=False):
            if model_choice in ['Prophet', 'Compare All Models']:
                ...
                kwargs['daily_seasonality']  = st.checkbox("Daily Seasonality",  value=False)
                kwargs['weekly_seasonality'] = st.checkbox("Weekly Seasonality", value=True)
                kwargs['yearly_seasonality'] = st.checkbox("Yearly Seasonality", value=True)

            if model_choice in ['ARIMA', 'Compare All Models']:
                st.markdown("#### ARIMA Settings")
                kwargs['auto_arima'] = st.checkbox("Use Auto-ARIMA", True)
            ...
            if model_choice in ['XGBoost', 'Compare All Models']:
                ...
                kwargs['n_estimators']    = st.slider("Trees (n_estimators)", 50, 500, 200, key='x2')
                kwargs['max_depth']       = st.slider("Max Depth", 2, 10, 3,               key='x3')
                kwargs['learning_rate']   = st.selectbox("Learning Rate", [0.01, 0.05, 0.1, 0.2], index=1)
                kwargs['subsample']       = st.slider("Subsample Ratio",  0.5, 1.0, 0.8,  key='x4')
                kwargs['colsample_bytree']   = st.slider("Feature Sample Ratio", 0.5, 1.0, 0.8, key='x5')
                kwargs['min_child_weight'] = st.slider("Min Child Weight", 1, 15, 3,       key='x6')
            ...
            if model_choice in ['Random Forest (ML Regressor)', 'Compare All Models']:
                ...
                kwargs['rf_n_estimators'] = st.slider("Trees (n_estimators)", 50, 500, 200, key='rf2')
            ...
            if model_choice in ['Exponential Smoothing', 'Compare All Models']:
                ...
                kwargs['seasonal']     = st.selectbox("Seasonal Component", ['add', 'mul', None], index=0)
...
def render_target_selection(numerical_cols, fallback_target):
    """
    Renders OHLC price selection as a vertical toggle list in the sidebar.
    Shows only Open, High, Low, Close when available; falls back to all numeric cols.
    """
    with st.sidebar:
        with st.expander("📂 Dataset Configuration", expanded=True):
            ohlc_available = [c for c in OHLC_OPTIONS if c in numerical_cols]
            options = ohlc_available if ohlc_available else numerical_cols

            default = fallback_target if fallback_target in options else options[0]
